...
public class ArrayListExample {
     ...
     */
	public static double total_area(ArrayList<? extends TwoDShape> sh){
		// wildcard extends TwoDshape to access area
		double total = 0;
		for(TwoDShape s:  sh) {
			total += s.area();
		}
     ...
     */
	public static double total_perimeter(ArrayList<? extends Rectangle> sh) {
		// wildcard extends Rectangle to access perimeter
		double total = 0;
		for(Rectangle s:  sh) {
			total += s.perimeter();
		}
     ...
     */
	public static void describe_all(ArrayList<? extends GeometricShape> sh) {
		// wildcard extends GeometricShape to get describe
		for(GeometricShape s:  sh) {
			s.describe();
		}
		System.out.println("Total number of shapes: " + sh.size());
     ...
     *      {@code new Sphere(0.0);          }
     */
	public static void add_empties(ArrayList<GeometricShape> sh) {
		// add new shape
		sh.add(new Cone(0.0, 0.0));
        sh.add(new Circle(0.0));
        sh.add(new Rectangle(0.0, 0.0));
        sh.add(new Sphere(0.0));
	}
	...
	public static <T> ArrayList<T> supersize_list(ArrayList<? extends GeometricShape> sh){
		// <T> is the returning type, wildcard extends GeometricShape to access supersize
		ArrayList<T> newShape = new ArrayList<T>();
		for(GeometricShape s:sh) {
			newShape.add(s.supersize());
		}
		return newShape;
	}
    ...
    public static void main(String[] args) {
        ...
        rects1.add(new Rectangle(2.0, 3.0));
        rects1.add(new Rectangle(5.0, 5.0));
        // Make a list of spheres
        ArrayList<Sphere> spheres1 = new ArrayList<Sphere>();
        spheres1.add(new Sphere(10.0));
        spheres1.add(new Sphere(50.0));
        spheres1.add(new Sphere(0.0));
        // Super-size them
        System.out.println();
        System.out.println("super-sizing a list of rectangles");
        ArrayList<Rectangle> double_rects = supersize_list(rects1);
        describe_all(double_rects);
        System.out.println();
        System.out.println("super-sizing a list of spheres");
        ArrayList<Sphere> double_spheres = supersize_list(spheres1);
