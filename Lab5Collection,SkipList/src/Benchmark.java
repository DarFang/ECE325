...
{
  /**
   * Modifies the given list, changing each element to the result of computing base power the element
  ...
  /**
   * Computes the position in the given {@code list} where the given {@code elem} is greater
   * than the current elements in the list.
   ...
   * benchmark with the given number of elements specified by {@code numElements},
   * and repeating each test {@code iteration} times
   * @param list        {@code List<Integer>} empty list to use
   * @param numElemens  {@code List<Integer>} list specifying the number of elements to use on each test
